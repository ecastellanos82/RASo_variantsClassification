...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(     
      ...
      textInput(inputId = "id", value = '11131', label = "Specify variant ID in Pandora"),
...
server <- function(input, output, session) {
  ...
  db_connect_postgres <- function(db_conf) {
    ...
    ## never add your pass, IP or user here; edit it by hand
 ...
 Automatic_criteria_AMCG<- function(id = input$id, con = con, denovo_noconfirmed = input$denovo_noconfirmed,
  ...
  Final_classificationB<- function(id = input$id, con = con, denovo_noconfirmed = input$denovo_noconfirmed,
    ...
    ##                     classification$Likely_PAT==1 & classification$Neutral==1 | classification$Likely_PAT==1 & classification$Likely_Neutral==1]<-1
      ...
      classification <- data.frame(head(iris, 5), id = rep(input$id, 5),
                                   timestamp = rep(date(), 5),
                                   go = rep(input$go, 5))
  ...
  #### VARIANT CONFIRMATION
      ...
      variant_reactive <- eventReactive(c(input$go, input$id, input$denovo_noconfirmed,
                                          input$cosegregation, input$PPAT_evidence,
                                          input$PPOL_evidence, input$Functional_evidence, input$go),
      {
          ...
          if (input$go == 0){
              ...
              return(data.frame(current_id= input$id, timestamp = date()))
          }
      })
  ...
  #### AUTOMATIC VARIANT CLASSIFICATION
      ...
      AutomClass_reactive <- eventReactive(c(input$go, input$id, input$denovo_noconfirmed,
                                          input$cosegregation, input$PPAT_evidence,
                                          input$PPOL_evidence, input$Functional_evidence,
                                          input$go),
      {
          Automatic_criteria_AMCG(id = input$id, con = con,
                                  denovo_noconfirmed = input$denovo_noconfirmed,
                                  denovo_confirmed = input$denovo_confirmed,
                                  cosegregation = input$cosegregation,
                                  PPAT_evidence = as.numeric(as.character(input$PPAT_evidence)), 
                                  PPOL_evidence = as.numeric(as.character(input$PPOL_evidence)),
                                  Functional_evidence = as.numeric(as.character(input$Functional_evidence)))
      }
  ...
  #### FINAL VARIANT CLASSIFICATION
      ...
      FinalClass_reactive <- eventReactive(
          c(input$go, input$id, input$denovo_noconfirmed,
            input$cosegregation, input$PPAT_evidence,
            input$PPOL_evidence, input$Functional_evidence, input$go), {
                Final_classificationB(id = input$id,
                                      con = con, denovo_noconfirmed = input$denovo_noconfirmed,
                                                           denovo_confirmed = input$denovo_confirmed, cosegregation = input$cosegregation,
                                                           PPAT_evidence = as.numeric(as.character(input$PPAT_evidence)), 
                                                           PPOL_evidence = as.numeric(as.character(input$PPOL_evidence)),
                                                           Functional_evidence = as.numeric(as.character(input$Functional_evidence)))})
